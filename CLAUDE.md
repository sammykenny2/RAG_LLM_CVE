...
**Format comparison**:
- `csv`: Largest file, slowest read, maximum compatibility
- `pkl` (default): Balanced size/speed, Python-native
- `parquet`: Smallest file, fastest read, requires `pip install pyarrow`
...
python theRag.py --extension=parquet  # Read CVEEmbeddings.parquet (fastest)
...
├── requirements.txt     # Python dependencies (includes pyarrow for parquet support)
├── CVEEmbeddings.pkl    # Generated by localEmbedding.py (default format)
│                        # (or .csv / .parquet depending on --extension parameter)
...
- `theRag.py` reads from:
  ...
  - `CVEEmbeddings.{extension}` (embedding database, based on `--extension` parameter)
    - `--extension=csv` → `CVEEmbeddings.csv`
    - `--extension=pkl` (default) → `CVEEmbeddings.pkl`
    - `--extension=parquet` → `CVEEmbeddings.parquet`
...
- **Parquet Support**: If using `--extension=parquet`, ensure `pyarrow` is installed: `pip install pyarrow`
