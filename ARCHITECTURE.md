...
- **Parameters**:
  ...
  - `--extension`: csv, pkl (default), or parquet
- **Process**:
  ...
  4. Save to selected format (csv/pkl/parquet) with columns: `sentence_chunk`, `embedding`
- **Performance**:
...
1. **CVEEmbeddings.{extension}**: Must exist before running `theRag.py`
   ...
   - Parquet format requires `pip install pyarrow`
