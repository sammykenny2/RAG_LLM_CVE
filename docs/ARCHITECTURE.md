...
- **Run once**: Before first use of cli/theRag.py
...
- **Optional Tools**: `cli/extractCVE.py` flattens JSONs to `CVEDescription<year>.txt` (for human reference only, not used by RAG)
...
### Main Implementation: cli/theRag.py
...
1. **CVEEmbeddings.{extension}**: Must exist before running `cli/theRag.py`
   - Generated by `cli/localEmbedding.py --extension={extension}`
...
# Specify speed level
python cli/theRag.py --speed=normal   # Baseline, maximum precision
python cli/theRag.py --speed=fast     # Recommended (default)
python cli/theRag.py --speed=fastest  # Maximum speed

# Combine with other parameters
python cli/theRag.py --speed=fastest --mode=demo
python cli/theRag.py --speed=fast --schema=v5
python cli/theRag.py --speed=normal --mode=full --schema=all
```
...
**Example**:
```bash
python cli/theRag.py --speed=fastest
```
...
```bash
python cli/theRag.py --speed=fast
```
