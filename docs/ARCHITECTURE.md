...
3. **LLM Call 2**: Call `asking_llama_for_advice()`:
   - Load `knowledge_base.*` (embedding database)
...
1. **knowledge_base.{extension}**: Must exist before running `cli/validate_report.py`
   - Generated by `cli/build_embeddings.py --extension={extension}`
   - Default: `knowledge_base.pkl` (file)
   - Chroma: `knowledge_base.chroma/` (directory)
...
**Problem**: v2 introduces auto-embedding (different from previous behavior). SessionManager creates files/sessions/ and embeddings/sessions/ directories even when not needed.
...
**Configuration**:
- `False` (default): Resource-efficient - no session directories creation, files only for special commands (summarize/validate)
- `True`: Multi-file context - files auto-embedded and searchable, creates files/sessions/ and embeddings/sessions/ directories
